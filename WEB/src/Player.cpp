...
///==== PLAYER ======///
    void PlayerInit(){
        Rectangle source = (Rectangle){static_cast<float>(playerobj.index*OBJ_SIZE),0,OBJ_SIZE,OBJ_SIZE};
        Rectangle dest = (Rectangle){static_cast<float>(GetScreenWidth()/2),static_cast<float>(GetScreenHeight()/2),OBJ_SIZE*2,OBJ_SIZE*2};
...
///==== SHOOT ====///
    void Bullets(){
        TimeforBullets--;
        //WEB ERROR: Dont reconociment the Key Press, Charge for IskeyDown or IsKeyUP
        if(IsKeyDown(KEY_SPACE) && playerobj.active && (!pclass.InShop) && (TimeforBullets < 5)){
            TimeforBullets = 8;
