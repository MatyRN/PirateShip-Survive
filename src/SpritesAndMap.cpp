...
#include "Shop.cpp"
    ...
    void SpritesAndMap::Update(){
        ...
        if(LV.StateGame==0){
            ...
            DrawTextureRec(SM.AutorSplashImage,sourceAutor,(Vector2){GetScreenWidth() / 3, TitleAnim() + 80},WHITE);
        }
        ...
        if(LV.StateGame==1){
            ...
            DrawTextureRec(SM.Title,sourceTitle,(Vector2){GetScreenWidth() / 18, TitleAnim()},WHITE);
            DrawText("Press START",GetScreenWidth() / 2.5,StartAnim(),20,WHITE);
        }
        ...
        //=====Draw shoot=====//
            for (int i = 0; i < PLAYER_MAX_SHOOTS; i++){
                    sourceSHOTS = (Rectangle){pclass.BulletTipe*OBJ_SIZE,0,OBJ_SIZE,OBJ_SIZE};
            ...
            for(LV.EnemysActives = 0; LV.EnemysActives < ENEMY_MAX; LV.EnemysActives++){
                if(enemy[LV.EnemysActives].active){
                        if(enemy[LV.EnemysActives].life < 1){
                         ...
                         sourceEnemy = (Rectangle){enemy[LV.EnemysActives].index*8,0,OBJ_SIZE,OBJ_SIZE};
        ...
        //DrawText(TextFormat("Waves: %i",LV.WavesCount),10,370,20,WHITE);
